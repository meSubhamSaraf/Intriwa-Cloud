...
type DbMechanic = { id: string; name: string; phone: string | null; employmentType: string };
type DbRecord   = { mechanicId: string; date: string; status: string; overtimeHours: number | null };

type DisplayMechanic = { id: string; name: string; skills: string[]; employmentType: string };
type DisplayRecord   = { mechanicId: string; date: string; status: AttendanceStatus; overtimeHours?: number };

const DB_STATUS_MAP: Record<string, AttendanceStatus> = {
  PRESENT: "present", HALF_DAY: "half_day", OVERTIME: "overtime", ABSENT: "absent",
};

function dbMechToDisplay(m: DbMechanic): DisplayMechanic {
  return { id: m.id, name: m.name, skills: [], employmentType: m.employmentType };
}

function dbRecordToDisplay(r: DbRecord): DisplayRecord {
  return {
    mechanicId: r.mechanicId,
    date: r.date.slice(0, 10),
    status: DB_STATUS_MAP[r.status] ?? "absent",
...
function AttendancePageInner() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([mechs, att]: [DbMechanic[], { records: DbRecord[] }]) => {
        setDbMechanics(mechs.map(dbMechToDisplay));
        setDbRecords(att.records.map(dbRecordToDisplay));
      })
  ...
  async function saveAttendance() {
    const entries = mechanics.map((m) => {
      ...
      const ot = overtimeHours[key] ?? getRecord(m.id, viewDate)?.overtimeHours;
      return { mechanicId: m.id, date: viewDate, status: status.toUpperCase(), overtimeHours: status === "overtime" ? (ot ?? 2) : undefined };
    });
    try {
      await Promise.all(entries.map((e) =>
        fetch("/api/attendance", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(e) })
      ));
      setSaved(true);
  ...
  return (
    <div className="p-4 max-w-5xl">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12 text-slate-400 text-sm">Loading attendance data…</div>
      ) : (
        <>
          {/* ── Daily view ─────────────────────────────────────────── */}
          {tab === "daily" && (
            <>
              ...
              <div className="bg-white border border-slate-200 rounded-xl overflow-hidden mb-4">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {mechanics.length === 0 ? (
                      <tr><td colSpan={4} className="px-4 py-8 text-center text-sm text-slate-400">No mechanics found in database.</td></tr>
                    ) : mechanics.map((m) => {
                      ...
                      return (
                        <tr key={m.id} className="border-b border-slate-100 last:border-0">
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2.5">
                              ...
                              <div>
                                ...
                                <p className="text-[10px] text-slate-400">{m.skills.join(" · ") || m.employmentType}</p>
                              </div>
                            </div>
                          </td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3">
                            <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded border ${m.employmentType === "employee" || m.employmentType === "FULL_TIME" || m.employmentType === "PART_TIME" ? "text-blue-700 bg-blue-50 border-blue-200" : "text-violet-700 bg-violet-50 border-violet-200"}`}>
                              {m.employmentType === "employee" || m.employmentType === "FULL_TIME" ? "Employee" : m.employmentType === "PART_TIME" ? "Part Time" : "Freelance"}
                            </span>
                          </td>
                        </tr>
                      );
          ...
          {tab === "monthly" && (
            <div>
              <div className="flex items-center gap-3 mb-5">
                ...
                <button
                  onClick={() => { if (monthAnchor < currentMonth) setMonthAnchor(addMonths(monthAnchor, 1)); }}
                  className={`w-7 h-7 flex items-center justify-center rounded border border-slate-200 transition-colors ${monthAnchor >= currentMonth ? "text-slate-300 cursor-not-allowed" : "text-slate-500 hover:bg-slate-50"}`}
                >
                  <ChevronRight className="w-4 h-4" />
                </button>
              ...
              <div className="space-y-4">
                {mechanics.map((m) => {
                  ...
                  const records = daysInMonth.map((d) => ({
                    ...
                    status: (allRecords.find((r) => r.mechanicId === m.id && r.date === d)?.status ?? "absent") as AttendanceStatus,
                  }));
                      ...
                      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                        ...
                        <div className="flex gap-2 flex-wrap">
                          ...
                          <span className="flex items-center gap-1 text-[11px] font-semibold text-blue-700 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-200">
                            <AlertCircle className="w-3 h-3" /> {otDays} OT{totalOtHrs > 0 ? ` (${totalOtHrs}h)` : ""}
                          </span>
