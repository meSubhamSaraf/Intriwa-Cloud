...
import {
  ...
  Fuel, Trash2, Package, Tag, RotateCcw, ChevronRight, ChevronDown,
} from "lucide-react";
...
type Tab = "profile" | "team" | "catalog" | "packages" | "skills" | "templates" | "whatsapp" | "hours" | "areas" | "operations";
...
type InvDropdownItem = { id: string; name: string; unitPrice: number; mrp: number | null };

type ServicePkg = {
  id: string; name: string; description: string | null;
  packagePrice: number; durationMinutes: number;
  isActive: boolean; items: PkgItem[]; createdAt: string;
};

function calcMrpTotal(items: PkgItem[]) {
  return items.reduce((s, i) => s + Number(i.mrpPrice) * Number(i.quantity), 0);
}

function emptyPkgItem(): PkgItem { return { description: "", mrpPrice: 0, quantity: 1 }; }

function PkgModal({ initial, onClose, onSave }: { initial: ServicePkg | null; onClose: () => void; onSave: (p: ServicePkg) => void }) {
  const isEdit = !!initial;
  const [name, setName] = useState(initial?.name ?? "");
  const [desc, setDesc] = useState(initial?.description ?? "");
  const [price, setPrice] = useState(initial ? String(Number(initial.packagePrice)) : "");
  const [duration, setDuration] = useState(String(initial?.durationMinutes ?? 60));
  const [items, setItems] = useState<PkgItem[]>(initial?.items.length ? initial.items.map(i => ({ ...i })) : [emptyPkgItem()]);
  const [saving, setSaving] = useState(false);
  const [invItems, setInvItems] = useState<InvDropdownItem[]>([]);

  useEffect(() => {
    fetch("/api/inventory").then(r => r.ok ? r.json() : []).then((d: InvDropdownItem[]) => setInvItems(d)).catch(() => {});
  }, []);

  const mrpTotal = calcMrpTotal(items);
  const priceNum = Number(price) || 0;

  function updItem(idx: number, field: keyof PkgItem, val: string | number) {
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      ...
      const payload = { name: name.trim(), description: desc.trim() || null, packagePrice: Number(price), durationMinutes: Number(duration) || 60, items: validItems.map(i => ({ description: i.description, mrpPrice: Number(i.mrpPrice), quantity: Number(i.quantity) || 1, inventoryItemId: i.inventoryItemId ?? null })) };
      ...
      const saved: ServicePkg = await res.json();
      onSave(saved);
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-slate-800 text-sm">{isEdit ? "Edit Package" : "New Service Package"}</h3>
          </div>
        ...
        <form onSubmit={save} className="flex-1 overflow-y-auto p-5 space-y-4">
          <div>
            ...
            <input required type="text" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Comprehensive Service" className="w-full h-9 px-3 text-sm border border-slate-200 rounded-md focus:outline-none focus:border-brand-navy-400" />
          </div>
          <div>
            ...
            <input type="text" value={desc} onChange={e => setDesc(e.target.value)} placeholder="e.g. Full-service for 4W petrol vehicles" className="w-full h-9 px-3 text-sm border border-slate-200 rounded-md focus:outline-none focus:border-brand-navy-400" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1.5">Package Price (₹) *</label>
              <input required type="number" min={0} value={price} onChange={e => setPrice(e.target.value)} placeholder="e.g. 4299" className="w-full h-9 px-3 text-sm border border-slate-200 rounded-md focus:outline-none focus:border-brand-navy-400" />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1.5">Duration (min)</label>
              <input type="number" min={15} step={15} value={duration} onChange={e => setDuration(e.target.value)} className="w-full h-9 px-3 text-sm border border-slate-200 rounded-md focus:outline-none focus:border-brand-navy-400" />
            </div>
          </div>
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs font-medium text-slate-600">Items</label>
              <button type="button" onClick={() => setItems(p => [...p, emptyPkgItem()])} className="flex items-center gap-1 text-[11px] font-medium text-brand-navy-600 hover:text-brand-navy-800"><Plus className="w-3 h-3" /> Add item</button>
            </div>
            <div className="space-y-2">
              {items.map((item, idx) => (
                <div key={idx} className="bg-slate-50 rounded-md px-3 py-2 space-y-1.5">
                  <div className="flex items-center gap-2">
                    ...
                    <select value={item.inventoryItemId ?? ""} onChange={e => {
                      ...
                      if (!sel) { updItem(idx, "inventoryItemId", ""); return; }
                      const found = invItems.find(i => i.id === sel);
                      if (found) setItems(prev => prev.map((it, i) => i !== idx ? it : { ...it, inventoryItemId: found.id, description: found.name, mrpPrice: found.mrp != null ? found.mrp : found.unitPrice }));
                    ...
                    {item.inventoryItemId && <button type="button" onClick={() => setItems(prev => prev.map((it, i) => i !== idx ? it : { ...it, inventoryItemId: null }))} className="text-slate-300 hover:text-slate-500 shrink-0"><X className="w-3 h-3" /></button>}
                  </div>
                  <div className="flex items-center gap-2">
                    <input type="text" value={item.description} onChange={e => updItem(idx, "description", e.target.value)} placeholder="Item description…" className="flex-1 h-7 px-2 text-xs border border-slate-200 rounded focus:outline-none focus:border-brand-navy-400 bg-white" />
                    <div className="flex items-center gap-1 shrink-0">
                      <span className="text-[10px] text-slate-400">MRP ₹</span>
                      <input type="number" min={0} value={item.mrpPrice || ""} onChange={e => updItem(idx, "mrpPrice", Number(e.target.value))} placeholder="0" className="w-16 h-7 px-1.5 text-xs border border-slate-200 rounded focus:outline-none focus:border-brand-navy-400 bg-white" />
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                      <span className="text-[10px] text-slate-400">Qty</span>
                      <input type="number" min={1} value={item.quantity} onChange={e => updItem(idx, "quantity", Number(e.target.value))} className="w-10 h-7 px-1.5 text-xs border border-slate-200 rounded focus:outline-none focus:border-brand-navy-400 bg-white" />
                    </div>
                    <button type="button" onClick={() => setItems(prev => prev.filter((_, i) => i !== idx))} className="text-slate-300 hover:text-red-500 shrink-0"><X className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
              ))}
            </div>
          </div>
          {(mrpTotal > 0 || priceNum > 0) && (
            <div className="bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 space-y-1">
              <div className="flex justify-between text-[12px]"><span className="text-slate-500">MRP Total</span><span className="text-slate-700 tabular-nums">₹{mrpTotal.toLocaleString("en-IN")}</span></div>
              <div className="flex justify-between text-[12px]"><span className="text-slate-500">Package Price</span><span className="font-semibold text-brand-navy-800 tabular-nums">{priceNum > 0 ? `₹${priceNum.toLocaleString("en-IN")}` : "—"}</span></div>
              <div className="flex justify-between text-[12px]"><span className="text-slate-500">Duration</span><span className="text-slate-700 tabular-nums">{Number(duration) || 60} min</span></div>
              {mrpTotal - priceNum > 0 && <div className="flex justify-between text-[12px]"><span className="text-green-600 font-medium">Customer Saves</span><span className="text-green-700 font-semibold tabular-nums">₹{(mrpTotal - priceNum).toLocaleString("en-IN")}</span></div>}
            </div>
          )}
        </form>
        <div className="px-5 py-4 border-t border-slate-100 flex gap-3 shrink-0">
          ...
          <button onClick={save as unknown as React.MouseEventHandler} disabled={saving || !name.trim() || !price} className="flex-1 h-10 bg-brand-navy-800 text-white text-sm font-medium rounded-xl hover:bg-brand-navy-700 disabled:opacity-60">
...
function PkgCard({ pkg, onEdit, onDeactivate, onReactivate }: { pkg: ServicePkg; onEdit: (p: ServicePkg) => void; onDeactivate: (id: string) => void; onReactivate: (id: string) => void }) {
    ...
    <div className={`bg-white border rounded-lg overflow-hidden ${pkg.isActive ? "border-slate-200" : "border-slate-100 opacity-70"}`}>
      <div className="p-4">
        <div className="flex items-start justify-between gap-3 mb-2">
          <div className="flex-1 min-w-0">
            <h3 className="text-sm font-semibold text-slate-800 truncate">{pkg.name}</h3>
            {pkg.description && <p className="text-[11px] text-slate-500 mt-0.5">{pkg.description}</p>}
            <p className="text-[10px] text-slate-400 mt-0.5">{pkg.durationMinutes} min</p>
          </div>
          <div className="text-right shrink-0">
            <div className="text-lg font-bold text-brand-navy-800">₹{Number(pkg.packagePrice).toLocaleString("en-IN")}</div>
            {mrpTotal > Number(pkg.packagePrice) && <div className="text-[11px] text-slate-400 line-through">₹{mrpTotal.toLocaleString("en-IN")}</div>}
          </div>
        </div>
        {savings > 0 && <div className="inline-flex items-center gap-1 bg-green-50 border border-green-200 text-green-700 text-[11px] font-semibold px-2 py-0.5 rounded mb-3"><Tag className="w-3 h-3" />Save ₹{savings.toLocaleString("en-IN")}</div>}
        {pkg.items.length > 0 && (
          <div className="mt-2 space-y-1">
            {pkg.items.map((item, i) => (
              <div key={item.id ?? i} className="flex items-center justify-between text-[12px]">
                ...
                <span className="text-slate-600 ml-3 shrink-0 tabular-nums">₹{Number(item.mrpPrice).toLocaleString("en-IN")}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="border-t border-slate-100 px-4 py-2.5 flex items-center justify-end gap-3">
        {pkg.isActive ? (
          <>
            <button onClick={() => onEdit(pkg)} className="flex items-center gap-1 text-[11px] font-medium text-brand-navy-600 hover:text-brand-navy-800"><Edit2 className="w-3 h-3" /> Edit</button>
            <button onClick={() => onDeactivate(pkg.id)} className="flex items-center gap-1 text-[11px] font-medium text-red-500 hover:text-red-700"><Trash2 className="w-3 h-3" /> Deactivate</button>
          </>
        ) : (
          <button onClick={() => onReactivate(pkg.id)} className="flex items-center gap-1 text-[11px] font-medium text-green-600 hover:text-green-800"><RotateCcw className="w-3 h-3" /> Reactivate</button>
        )}
      </div>
    </div>
  );
...
function PackagesTab() {
  const [pkgs, setPkgs] = useState<ServicePkg[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editing, setEditing] = useState<ServicePkg | null>(null);
  const [showInactive, setShowInactive] = useState(false);

  useEffect(() => {
    fetch("/api/packages").then(r => r.ok ? r.json() : []).then((d: ServicePkg[]) => setPkgs(d)).catch(() => setPkgs([])).finally(() => setLoading(false));
  }, []);

  function handleSaved(saved: ServicePkg) {
    setPkgs(prev => { const idx = prev.findIndex(p => p.id === saved.id); if (idx >= 0) { const n = [...prev]; n[idx] = saved; return n; } return [saved, ...prev]; });
    setShowModal(false);
    setEditing(null);
  }
  ...
  async function handleDeactivate(id: string) {
    ...
    if (res.ok) { setPkgs(prev => prev.map(p => p.id === id ? { ...p, isActive: false } : p)); toast.success("Package deactivated"); }
    else toast.error("Failed to deactivate");
  }

  async function handleReactivate(id: string) {
    const res = await fetch(`/api/packages/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ isActive: true }) });
    if (res.ok) { const u: ServicePkg = await res.json(); setPkgs(prev => prev.map(p => p.id === id ? u : p)); toast.success("Package reactivated"); }
    else toast.error("Failed to reactivate");
  }

  const active = pkgs.filter(p => p.isActive);
  const inactive = pkgs.filter(p => !p.isActive);

  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <SectionLabel>Service Packages ({active.length} active)</SectionLabel>
        <button onClick={() => { setEditing(null); setShowModal(true); }} className="flex items-center gap-1.5 text-sm font-medium bg-brand-navy-800 text-white hover:bg-brand-navy-700 px-3 py-1.5 rounded-md transition-colors">
          <Plus className="w-3.5 h-3.5" /> New Package
        </button>
      </div>

      {loading ? (
        <div className="text-center py-10 text-sm text-slate-400"><Loader2 className="w-4 h-4 animate-spin inline mr-2" />Loading…</div>
      ) : active.length === 0 && inactive.length === 0 ? (
        <div className="text-center py-12">
          <Package className="w-10 h-10 text-slate-200 mx-auto mb-2" />
          <p className="text-sm font-medium text-slate-600">No packages yet</p>
          <p className="text-[12px] text-slate-400 mt-1">Create your first service package to offer bundled pricing.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {active.map(pkg => <PkgCard key={pkg.id} pkg={pkg} onEdit={p => { setEditing(p); setShowModal(true); }} onDeactivate={handleDeactivate} onReactivate={handleReactivate} />)}
          {inactive.length > 0 && (
            <div className="mt-4">
              <button onClick={() => setShowInactive(v => !v)} className="flex items-center gap-2 text-[11px] font-semibold text-slate-500 uppercase tracking-wide hover:text-slate-700 mb-3">
                {showInactive ? <ChevronDown className="w-3.5 h-3.5" /> : <ChevronRight className="w-3.5 h-3.5" />} Inactive ({inactive.length})
              </button>
              {showInactive && <div className="space-y-3">{inactive.map(pkg => <PkgCard key={pkg.id} pkg={pkg} onEdit={p => { setEditing(p); setShowModal(true); }} onDeactivate={handleDeactivate} onReactivate={handleReactivate} />)}</div>}
            </div>
          )}
        </div>
      )}

      {showModal && <PkgModal initial={editing} onClose={() => { setShowModal(false); setEditing(null); }} onSave={handleSaved} />}
    </div>
  );
}
